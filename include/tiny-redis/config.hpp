#ifndef __TINY_REDIS_CONFIG_HPP__
#define __TINY_REDIS_CONFIG_HPP__
#include <string>
...
namespace tiny_redis {
    ...
    struct AofOptions
    {
        ...
        int batch_wait_us = 1500;                 // 聚合等待上限(ms)
        size_t prealloc_bytes = 64 * 1024 * 1024; // 初始预分配大小
        int sync_interval_ms = 1000;              // everysec实际同步周期(ms),可调平滑尾延迟
        // optional smoothing knobs (Linux only)
        bool use_sync_file_range = false;         // 写入后触发后台回写(SFR_WRITE)
        size_t sfr_min_bytes = 512 * 1024;        // 达到该批量再调用sync_file_range, 避免过于频繁
        bool fadvise_dontneed_after_sync = false; // 每次fdatasync后对已同步范围做DONTNEED
    };

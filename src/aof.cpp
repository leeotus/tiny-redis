...
namespace tiny_redis {
    ...
    // @brief 将输入的d命令字符串向量转换为RESP格式的字符串
    ...
    bool AofLogger::init(const AofOptions &opts, std::string &err)
    {
        ...
        // 创建文件并以只写模式追加, 设定文件所有者读写权限, 文件所属组读权限和其他用户读权限
        ...
        if(opts_.prealloc_bytes > 0) {
            /**
             * @brief 预分配指定大小的文件空间
             * @note 作用: 1. 避免文件在增长过程中多次进行磁盘分配操作, 减少文件碎片, 提高后续的写入性能
             * 2. 保证空间可用, 确保有足够的磁盘空间可用于后续写入, 避免在写入过程中因磁盘空间不足而导致失败
             * 3. 减少元数据更新, 预分配可以减少文件系统元数据的更新次数, 提高整体性能
             */
    ...
    void AofLogger::shutdown()
    {
        ...
        if(fd_ >= 0) {
            // @note 调用fdatasync来将所关联的文件的已修改数据刷新到物理存储设备上
            ::fdatasync(fd_);
            /**
             * @note
             * 1. sync: 将所有修改过的数据排入写队列然后返回, 不等待实际写磁盘操作的结束
             * 2. fsync: 等待磁盘操作完成并返回(数据+元数据)
             * 3. fdatasynce: 类似fsync, 但是它只影响文件的数据部分, 而不会更新元数据
             * 4. fflush: 标准C函数, 只刷新用户空间缓冲区, 不保证数据写入磁盘
             */
            ::close(fd_);
            fd_ = -1;
        }
    ...
    bool AofLogger::load(KeyValueStore &store, std::string &err)
    {
        ...
        std::string data;
        while(true) {
            // 一次性将AOF文件的数据全部写入到buf缓冲区中
            // TODO: 可以修改成每次读取多行然后执行后再读取, 以此循环
        ...
        auto readLine = [&](std::string &out) -> bool {
            ...
            // @note assign第一个参数表示数据复制的起始位置, 第二个参数表示需要复制的数据的长度

...
namespace tiny_redis {
    ...
    bool loadConfigFromFile(const std::string &path, ServerConfig &cfg, std::string &err)
    {
        ...
        {
            ++lineno;
            std::string t = trim(line); // 删除字符串两端的空格
            if (t.empty() || t[0] == '#')
            {
                // 如果为空或者是注释, 则跳过
                continue;
            ...
            {
                err = "invalid line: " + std::to_string(lineno) + ", please check!";
                return false;
            }
            // 获取当前行的配置:
            ...
            if (key == "port")
            {
                ...
                {
                    err = "invalid port at line: " + std::to_string(lineno) + " please check!";
                    return false;
                }
            }
            ...
            else if (key == "aof.mode")
            {
                ...
                {
                    err = "invalid aof.mode at line: " + std::to_string(lineno) + ", aof.mode must be one of 'no', 'everysec', or 'always'";
                    return false;
                }
            }
            else if (key == "aof.dir")
